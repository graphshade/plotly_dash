...
df = pd.read_excel('c:/users/elvis/documents/projects/superstore_dashboard/data/Superstore.xls',
                   sheet_name = 'Orders',
...
header = html.Div(className = 'header',
                  children = [
                      html.Div(className = 'header__title',
                               children = [
                                   html.H1('SuperStore Dashboard')]),
                      html.Div(className = 'header__logo',
                               children = [
                                   html.Img(src='assets/logo.svg', id = 'header__logo__img')])
                                     ])    
    
#app layout - filter
filters = html.Div(className = 'filter',
                   children = [
                       html.Div(className = 'filter__daterange',
                                children=[
                                    html.Div(
                       ...
                       html.Div(className = 'filter__compare',
                                children = [
                                    html.P('Compare to'),
                                    dcc.Dropdown(
                                        id = 'filter__compare__to_id',
                                        className = 'filter__compare__to',
                                        options = [{'label':'Prior Year', 'value':'prioryear'},
                       ...
                       html.Div(className = 'filter__submit',
                                children = [
...
#app label - bans
label_bans = html.Div(className = 'label__1',
                      children = [html.P('KPIs')])
                     

#app layout - Bans
bans = html.Div(className = 'bans',
                children = [
                    html.Div(className = 'bans__sales',
                             children = [
                                 html.H2(id = 'bans__sales__amount_id',
                                        className = 'bans__sales__amount',
                                         children = ['$235.3b']),
                                 html.H3(className = 'bans__sales__indicator',
                                         children=[
                                            html.Span(id = 'bans__sales__indicator_id'),
                                             html.Span(id = 'bans__sales__change_id',
                                                    className = 'bans__sales__change',
                                                       children = ['2%'])
                                             ]),
                                 html.P(className = 'bans__sales_label',
                                         children=['sales']),
                    ...
                    html.Div(className = 'bans__profit',
                             children = [
                                 html.H2(id = 'bans__profit__amount_id',
                                        className='bans__profit__amount',
                                         children = ['$101b']),
                                 html.H3(className='bans__profit__indicator',
                                         children=[
                                             html.Span(id = 'bans__profit__indicator_id'),
                                             html.Span(id = 'bans__profit__change_id',
                                                    className = 'bans__profit__change',
                                                       children = [str('12%')])]),
                                 html.P(className = 'bans__profit_label',
                                         children = ['profit']),
                                 dcc.Graph(id = 'bans__profit__trend_id', config = {'displayModeBar':False})
                                 ]),
                    
                    html.Div(className = 'bans__shipping',
                             children =[
                                 html.H2(id = 'bans__shipping__amount_id',
                                        className = 'bans__shipping__amount',
                                         children=['3.5 days']),
                                 html.H3(className = 'bans__shipping__indicator',
                                         children=[
                                             html.Span(id = 'bans__shipping__indicator_id'),
                                             html.Span(id = 'bans__shipping__change_id',
                                                    className = 'bans__shipping__change',
                                                       children=['15%'])
                                             ]),
                                 html.P(className = 'bans__shipping__label',
                                         children=['avg. shipping days']),
                                dcc.Graph(id = 'bans__shipping__trend_id', config = {'displayModeBar':False})
                                 ])                
...
#app label - content
label_content = html.Div(className = 'label__2',
                         children= [
                             html.P('Details')])
...
@app.callback([Output('bans__sales__amount_id','children'),
...
def update_metrics(n_clicks,start_date, end_date, comparison):
    ...
    if (s_date - daterange) >= df['Order Date'].min():
        df_pp = df[(df['Order Date']>= (s_date - daterange)) 
    ...
    return sales_cc_f, profit_cc_f, shipping_cc_f, sales_pct_label, profit_pct_label, shipping_pct_label, sales_pct_f, profit_pct_f, shipping_pct_f




#callback function to render the last n-days(10days) trend in a selected metric (sales, profit,shiping)
@app.callback([Output('bans__sales__trend_id','figure'),
...
def update_sparkline(n_clicks,e_date):
    ...
    def sparkline(df,metric,period,e_date):
        ...
        return (fig.update_xaxes(visible = False, fixedrange = True)
        .update_yaxes(visible = False, fixedrange = True)
        .update_layout(hovermode= False,annotations = [], overwrite = True, showlegend = False,plot_bgcolor = 'rgba(0,0,0,0)',paper_bgcolor = 'rgba(0,0,0,0)',margin=dict(l=2,t=2,b=2,r=2)))
